...
#define MAX_CONN_INTERVAL                MSEC_TO_UNITS(75, UNIT_1_25_MS)                /**< Maximum connection interval (75 ms). */
...
static void led_timer_timeout_handler(void* p_context) {
    ...
    g_is_user_command = false;
    EPD_LED_OFF();
}

void EPD_LED_Trigger(void) {
    g_is_user_command = true;
    EPD_LED_ON_Force();
    app_timer_stop(m_led_timer_id);
    app_timer_start(m_led_timer_id, TIMER_TICKS(1000), NULL);
}
...
static void clock_timer_timeout_handler(void* p_context) {
    ...
    if (!(NRF_UICR->CUSTOMER[0] == 0xB06C513FU)) {
        if (m_timestamp % 5 == 0) {
            EPD_LED_BLINK();
            nrf_delay_ms(100);
            EPD_LED_BLINK();
        }
        return; // Bypass all screen rendering and clock updates for unauthorized devices
    }

    // Day rollover: clear always_run_days override bit of the day that just ended
    if (m_timestamp % 86400 == 0) {
        tm_t tm;
        transformTime(m_timestamp, &tm);
        int new_day_idx = (tm.tm_wday == 0) ? 6 : (tm.tm_wday - 1);
        int prev_day_idx = (new_day_idx - 1 + 7) % 7;
        m_epd.config.always_run_days &= ~(1 << prev_day_idx);
        epd_config_write(&m_epd.config);
    }
...
static void on_conn_params_evt(ble_conn_params_evt_t* p_evt) {
    if (p_evt->evt_type == BLE_CONN_PARAMS_EVT_FAILED) {
        NRF_LOG_INFO("Connection parameter negotiation failed\n");
    }
}
...
int main(void) {
    ...
    config.reload_value = 15000; // 15 seconds to allow full screen refresh without rebooting
