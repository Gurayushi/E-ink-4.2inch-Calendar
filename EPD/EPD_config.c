...
void epd_timetable_read(timetable_data_t* tt) {
    ...
    memset(tt, 0, sizeof(timetable_data_t));
    memset(&ftok, 0x00, sizeof(fds_find_token_t));

    if (fds_record_find(TIMETABLE_FILE_ID, TIMETABLE_REC_KEY, &record_desc, &ftok) != NRF_SUCCESS) {
        NRF_LOG_DEBUG("epd_timetable_read: record not found\n");
        return;
    }
    if (fds_record_open(&record_desc, &flash_record) != NRF_SUCCESS) {
        NRF_LOG_ERROR("epd_timetable_read: record open failed!");
        return;
    }
#ifdef S112
    uint32_t record_len = flash_record.p_header->length_words * sizeof(uint32_t);
    ...
    uint32_t record_len = flash_record.p_header->tl.length_words * sizeof(uint32_t);
#endif
    memcpy(tt, flash_record.p_data, MIN(sizeof(timetable_data_t), record_len));
    fds_record_close(&record_desc);
}
...
void epd_timetable_write(timetable_data_t* tt) {
    ...
    record.file_id = TIMETABLE_FILE_ID;
    record.key = TIMETABLE_REC_KEY;
#ifdef S112
    record.data.p_data = (void*)tt;
    record.data.length_words = BYTES_TO_WORDS(sizeof(timetable_data_t));
#else
    fds_record_chunk_t record_chunk;
    record_chunk.p_data = tt;
    record_chunk.length_words = BYTES_TO_WORDS(sizeof(timetable_data_t));
    record.data.p_chunks = &record_chunk;
    record.data.num_chunks = 1;
#endif

    memset(&ftok, 0x00, sizeof(fds_find_token_t));
    ret = fds_record_find(TIMETABLE_FILE_ID, TIMETABLE_REC_KEY, &record_desc, &ftok);
    if (ret == NRF_SUCCESS)
        ret = fds_record_update(&record_desc, &record);
    else
        ret = fds_record_write(&record_desc, &record);
    ...
    if (ret != NRF_SUCCESS) {
        NRF_LOG_ERROR("epd_timetable_write: record write/update failed, code=%d\n", ret);
        if (ret == FDS_ERR_NO_SPACE_IN_FLASH) app_sched_event_put(NULL, 0, run_fds_gc);
    }
}
