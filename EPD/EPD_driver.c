...
volatile bool g_is_user_command = false;

// lED
void EPD_LED_ON(void) {
    if (g_is_user_command && EPD_LED_PIN != 0xFF) digitalWrite(EPD_LED_PIN, LOW);
}
...
void EPD_LED_Toggle(void) {
    if (g_is_user_command && EPD_LED_PIN != 0xFF) nrf_gpio_pin_toggle(EPD_LED_PIN);
}

void EPD_LED_ON_Force(void) {
    if (EPD_LED_PIN != 0xFF) {
...
static epd_model_t* epd_models[] = {
    &epd_ssd1619_420_bwr,  &epd_ssd1619_420_bw,   &epd_ssd1677_750_bwr, &epd_ssd1677_750_bw,
};
